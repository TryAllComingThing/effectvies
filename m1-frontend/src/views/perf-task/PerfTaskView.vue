<template>
  <PageContainer title="任务管理">
    ...
    <el-form inline :model="query" class="query-row">
      ...
      <el-form-item label="状态"><el-select v-model="query.taskStatus" clearable style="width: 120px"><el-option label="待开始" value="pending" /><el-option label="运行中" value="running" /><el-option label="已停止" value="stopped" /><el-option label="已完成" value="done" /><el-option label="失败" value="fail" /></el-select></el-form-item>
    ...
    <el-table v-loading="loading" :data="rows" size="small" @selection-change="onSelectionChange">
      <el-table-column type="selection" width="45" /><el-table-column type="index" width="56" label="#" /><el-table-column prop="taskNo" label="任务号" min-width="180" /><el-table-column prop="batchNo" label="批号" min-width="180" /><el-table-column prop="fileName" label="文件名称" min-width="120" />
      <el-table-column label="状态" width="120"><template #default="scope"><StatusTag :status="scope.row.taskStatus" /></template></el-table-column>
      <el-table-column label="进度" min-width="170"><template #default="scope"><el-progress :percentage="scope.row.progress" :status="scope.row.taskStatus === 'fail' ? 'exception' : undefined" /></template></el-table-column>
      <el-table-column prop="createdAt" label="创建时间" min-width="160" />
      <el-table-column label="操作" min-width="380" fixed="right"><template #default="scope"><el-space>
        <el-button text type="primary" size="small" v-permission="['admin']" @click="runMainAction(scope.row)"><ActionIcon :name="getMainActionMeta(scope.row).icon" />{{ getMainActionMeta(scope.row).label }}</el-button>
        <el-button text type="primary" size="small" v-permission="['admin']" @click="openEdit(scope.row)"><ActionIcon name="Pencil" />编辑</el-button>
        <el-button text size="small" v-permission="['admin']" @click="openLogDialog(scope.row)"><ActionIcon name="FileText" />日志</el-button>
        <el-button text type="danger" size="small" v-permission="['admin']" @click="runAction('clear', scope.row.id)"><ActionIcon name="Eraser" />清除</el-button>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogMode==='create'?'新增任务':'编辑任务'" width="480px">
      <el-form :model="form" label-width="90px">
        <el-form-item label="批号"><el-select v-model="form.batchNo" :disabled="dialogMode==='edit'" style="width:100%" placeholder="请选择已上传文件批号"><el-option v-for="item in batchNoOptions" :key="item.batchNo" :label="`${item.batchNo}（${item.name}）`" :value="item.batchNo" /></el-select></el-form-item>
        <el-form-item label="任务名称"><el-input v-model="form.fileName" /></el-form-item>
        <el-form-item label="状态"><el-select v-model="form.taskStatus" style="width:100%"><el-option label="待开始" value="pending" /><el-option label="运行中" value="running" /><el-option label="已停止" value="stopped" /></el-select></el-form-item>
      </el-form>
      <template #footer><el-button @click="dialogVisible=false">取消</el-button><el-button type="primary" @click="submitDialog">确定</el-button></template>
    </el-dialog>
...
const onSelectionChange = (s: PerfTaskItem[]) => { selectedIds.value = s.map((i) => i.id); };
const resetQuery = () => { Object.assign(query, { pageNum: 1, pageSize: 10, taskNo: '', batchNo: '', fileName: '', taskStatus: '' }); loadData(); };
...
const submitDialog = () => {
  if (!form.batchNo) { ElMessage.warning('请选择批号'); return; }
...
const runAction = async (action: 'start' | 'stop' | 'resume' | 'clear', id: string) => {
  const m = { start: startPerfTask, stop: stopPerfTask, resume: resumePerfTask, clear: clearPerfTask };
  const r = await m[action](id);
  if (r.code === 0) { ElMessage.success('操作成功'); loadData(); return; }
...
const openLogDialog = (row: PerfTaskItem) => {
  ...
  Object.assign(logSummary, { taskNo: row.taskNo, batchNo: row.batchNo, matchedCount, totalCount, progress: row.progress, status: row.taskStatus });

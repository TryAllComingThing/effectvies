<template>
  <PageContainer title="绩效审核管理">
    ...
    <el-dialog v-model="rejectVisible" title="驳回审核" width="420px">
      <el-form>
        <el-form-item label="驳回意见" required><el-input v-model="rejectComment" type="textarea" :rows="4" /></el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="rejectVisible = false">取消</el-button>
        <el-button type="primary" @click="runReject">确认</el-button>
      </template>
    </el-dialog>
...
import { approveReview, getReviewList, rejectReview } from '@/api/review';
import type { PerfReviewItem } from '@/types';

const loading = ref(false);
const rows = ref<PerfReviewItem[]>([]);
const total = ref(0);
...
const resetQuery = () => {
  Object.assign(query, { pageNum: 1, pageSize: 10, title: '', routeName: '', deptName: '', proposer: '' });
  loadData();
};

const onSelectionChange = (s: PerfReviewItem[]) => {
...
const runReject = async () => {
  if (!rejectComment.value.trim()) return ElMessage.warning('驳回意见必填');
  const r = await rejectReview(rejectTargetId.value, rejectComment.value);
  if (r.code === 0) {
